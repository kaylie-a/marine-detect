...
def evaluate_models(model_folders, iou_threshold=0.5):
    ...
    for model_name, paths in model_folders.items():
        ...
        precision_arr.append(avg_precision)
        recall_arr.append(avg_recall)
        f1_arr.append(avg_f1)
...
for i, j, k, l in zip(precision_arr, recall_arr, f1_arr, accuracy_arr):
    total1 += i
    total2 += j
    total3 += k
    total4 += l

print("\n\nOverall results for models:\n")
print(f"Average Precision: {(total1 / length):.2f}")
print(f"Average Recall:    {(total2 / length):.2f}")
print(f"Average F1 Score:  {(total3 / length):.2f}")
print(f"Average Accuracy:  {(total4 / length):.2f}")
